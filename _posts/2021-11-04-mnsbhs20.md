...
(For regression, we instead think of the labels as $$\langle w^*, x\rangle = x_t$$.)

Their theoretical results rely on having no label noise.

From this data model alone, we can plug in the bounds of BLLT19 to see what they tell us. _Note: There actually isn't a perfect analogue here, because BLLT includes additive label noise with variance $$\sigma^2$$. The purpose of these bounds is to illustrate what is known about a similar model._
...
They have two types of main results: Theorem 1 shows the sufficient conditions for the coincidence the SVM and OLS weights $$w_{SVM}$$ and $$w_{OLS}$$, and Theorem 2 analyzes the generalization of the excess errors of both classification and regression.
...
* One might observe that this doesn't have the other requirement from BLLT: that the variances do not decay too gradually, which is enforced by $$r + 2q > 1$$. This is absent here because this paper does not consider label noise, so the risk of a model being corrupted by overfitting noisy labels is minimized.
